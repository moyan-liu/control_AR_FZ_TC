...
def main():
    ...
    # =========================================================================
    # SELECT PERTURBATION LOCATIONS - SITE 3 ONLY
    # =========================================================================
    print("=" * 80)
    print("  Step 2: Use Site 3 Perturbation Location Only")
    print("=" * 80)
    print()

    # Hardcode Site 3 location: 14.25°N, 269°E
    from shared.metrics import calculate_distance

    selected_lats = [14.25]
    selected_lons = [269.0]

    # Verify distance from TC
    dist = calculate_distance(CONFIG['init_lat'], CONFIG['init_lon'],
                             selected_lats[0], selected_lons[0])

    print(f"Using only Site 3:")
    print(f"  Location: {selected_lats[0]:.2f}°N, {selected_lons[0]:.2f}°E")
    print(f"  Distance from TC: {dist:.0f} km")
    print()

    # Create seeding locations (single site)
    # Note: The function expects 1-indexed values (1, 2, 3...), not 0-indexed!
    seeding_locations = create_seeding_locations_from_candidates(
        selected_lats, selected_lons,
        selected_indices=[1],  # 1 = first location (function converts to 0-based internally)
    ...
    torch.save(preds_baseline, CONFIG['output_dir'] / 'preds_baseline_site3.pt')
    print(f"  ✓ Saved: {CONFIG['output_dir'] / 'preds_baseline_site3.pt'}")
    print()
    ...
    torch.save(preds_seeded, CONFIG['output_dir'] / 'preds_seeded_site3.pt')
    print(f"  ✓ Saved: {CONFIG['output_dir'] / 'preds_seeded_site3.pt'}")
    print()
    ...
    track_plot = CONFIG['output_dir'] / "sandy_ftle_track_comparison_site3.png"
    plot_custom_track_comparison(
        ...
        title="Sandy 2012 - Site 3 Only Perturbation Test Result",
    ...
    try:
        plot_seeding_locations_map(
            ...
            CONFIG['init_lat'],
            CONFIG['init_lon'],
            save_path=CONFIG['output_dir'] / "seeding_locations_map_site3.png"
        )
        print(f"✓ Saved seeding map: {CONFIG['output_dir'] / 'seeding_locations_map_site3.png'}")
