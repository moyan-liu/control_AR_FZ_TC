...
def plot_ftle_orthographic(seeding_locations, ftle_field, ftle_lats, ftle_lons,
    ...
    # CENTER THE ORTHOGRAPHIC VIEW ON THE NORTHERN HEMISPHERE
    # central_longitude adjusted to 282 for Atlantic (Sandy region)
    fig, ax = plt.subplots(
        figsize=(16, 16),
        subplot_kw={'projection': ccrs.Orthographic(central_longitude=282, central_latitude=30)}
    )

    # Show the visible hemisphere only
    ax.set_global()
    ...
    # This blanks out FTLE south of the equator so only the northern hemisphere FTLE is shown.
    ftle_plot = np.array(ftle_field, dtype=float)  # copy
    mask_south = LAT_FTLE < 0.0
    ftle_plot[mask_south] = np.nan

    # ---- STYLING: use YlOrRd (yellow-orange-red) cmap ----
    cmap = plt.colormaps.get_cmap('YlOrRd')
    vmin = np.nanpercentile(ftle_plot, 5)
    vmax = np.nanpercentile(ftle_plot, 95)
    levels_ftle = np.linspace(vmin, vmax, 100)

    # ========== LAYER 2: MAP FEATURES (BASE - BEFORE FTLE) ==========
    print("  Adding map base features...")
    ax.add_feature(cfeature.OCEAN, facecolor='white', alpha=1.0, zorder=1)
    ax.add_feature(cfeature.LAND, facecolor='lightgray', alpha=0.5, zorder=2)

    # ========== FTLE FIELD ON TOP OF BASE MAP ==========
    cf = ax.contourf(
        LON_FTLE, LAT_FTLE, ftle_plot,
    ...
    # ========== LAYER 3: COASTLINES AND BORDERS ON TOP ==========
...
def main():
    ...
    # Show all 10 candidates from the selection
    # No distance filtering - just take top 10 from FTLE selection
    selected_lats = selected_lats[:10]
    selected_lons = selected_lons[:10]
