...
def main():
    ...
    for i, loc in enumerate(seeding_locations):
        ax.plot(loc['lon_center'], loc['lat_center'],
               ...
               transform=ccrs.PlateCarree(),
               label='Seeding Cluster' if i == 0 else '',
               zorder=9)

    # Cluster center
    ax.plot(CLUSTER_CENTER['lon'], CLUSTER_CENTER['lat'],
           'r*', markersize=18, markeredgecolor='white', markeredgewidth=2,
           transform=ccrs.PlateCarree(),
           label=f'Cluster Center\n({CLUSTER_CENTER["lat"]:.1f}°N, {360-CLUSTER_CENTER["lon"]:.1f}°W)',
           zorder=11)

    ax.set_title('Ian 2022 - Targeted Cluster Seeding Test', fontsize=14, fontweight='bold', pad=10)
