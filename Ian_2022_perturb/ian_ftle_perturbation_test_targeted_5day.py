#!/usr/bin/env python3
"""
Hurricane Ian 2022 - FTLE-Guided Cloud Seeding (3-SITE TRAJECTORY)

Based on test findings: Original "wrong" location (20.0°N, 285.0°E) performed better
than the "correct" Site 2 location (37.3 km vs 27.8 km deviation).

Strategy:
1. Create 3 perturbation sites along a northeast trajectory
2. Sites positioned progressively closer to TC initial position
3. Each site has 300 km radius
4. Test if trajectory-based seeding improves upon single-site approach

Site Configuration:
- Site 1: 12.25°N, 278.75°E (81.25°W) - Starting point
- Site 2: 13.25°N, 280.25°E (79.75°W) - Middle point
- Site 3: 14.25°N, 281.25°E (78.75°W) - Closest to TC

Initialization: 5-day lead time (Sep 23, 2022 18:00 UTC)
Target: Sep 28, 2022 landfall (Florida)

Author: Adapted from ian_ftle_perturbation_test_targeted.py
Date: 2025-12-05
"""
...
def main():
    ...
    CONFIG = {
        ...
        'output_dir': Path("/scratch/qhuang62/control_AR_FZ_TC/Ian_2022_perturb/ian_ftle_test_output_targeted_5day"),
    }

    CONFIG['output_dir'].mkdir(exist_ok=True, parents=True)

    # Three-site trajectory configuration (based on analysis showing original location was better)
    # Sites along a northeast trajectory toward TC
    TRAJECTORY_SITES = [
        {'lat': 12.25, 'lon': 278.75, 'radius_km': 300},  # Site 1: Starting point
        {'lat': 13.25, 'lon': 280.25, 'radius_km': 300},  # Site 2: Middle point
        {'lat': 14.25, 'lon': 281.25, 'radius_km': 300},  # Site 3: Closest to TC
    ]
    ...
    print()
    print(f"⭐ 3-Site Trajectory Configuration:")
    for i, site in enumerate(TRAJECTORY_SITES, 1):
        print(f"   Site {i}: {site['lat']:.2f}°N, {site['lon']:.2f}°E ({360-site['lon']:.2f}°W) - Radius: {site['radius_km']} km")
    print()
    ...
    # =========================================================================
    # CREATE TRAJECTORY SEEDING SITES
    # =========================================================================
    print("=" * 80)
    print("  Step 2: Create 3-Site Trajectory Seeding Locations")
    print("=" * 80)
    print()

    # Create seeding locations directly from trajectory sites
    seeding_lats = [site['lat'] for site in TRAJECTORY_SITES]
    seeding_lons = [site['lon'] for site in TRAJECTORY_SITES]
    seeding_radii = [site['radius_km'] for site in TRAJECTORY_SITES]

    # Create seeding locations
    seeding_locations = []
    for i, (lat, lon, radius) in enumerate(zip(seeding_lats, seeding_lons, seeding_radii), 1):
        dist_from_tc = calculate_distance(CONFIG['init_lat'], CONFIG['init_lon'], lat, lon)
        ...
        print(f"Site {i}:")
        print(f"  Location: {lat:.2f}°N, {lon:.2f}°E ({360-lon:.2f}°W)")
        print(f"  Radius: {radius} km")
        print(f"  Distance from TC: {dist_from_tc:.1f} km")
        print()

    print(f"Total seeding sites: {len(seeding_locations)}")
    print()
    ...
    # =========================================================================
    print("=" * 80)
    print("  Step 5: Apply 3-Site Trajectory Seeding")
    print("=" * 80)
    print()
    ...
    # =========================================================================
    print("=" * 80)
    print("  Step 6: Run Trajectory-Seeded Forecast")
    print("=" * 80)
    print()
    ...
    print("Running trajectory-seeded forecast...")
    preds_seeded = run_forecast(
        model, batch_seeded, tracker_seeded,
        steps=CONFIG['steps'],
        name="Ian 3-Site Trajectory",
        device=device,
        verbose=True
    )
    ...
    print("Track Deviations (Baseline vs 3-Site Trajectory):")
    if len(deviations) > 4:
    ...
    print("Comparison with previous Ian tests:")
    print(f"  • Old targeted (wrong center 20.0°N, 285.0°E): 37.3 km")
    print(f"  • New trajectory (3 sites): {deviations[-1]:.1f} km")
    if deviations[-1] > 37.3:
        ...
        print(f"  ⚠️  Less effective: -{decrease:.1f} km ({decrease/37.3*100:.1f}% reduction)")
    print()
    ...
    print("Creating track comparison plot...")
    import matplotlib.pyplot as plt
    import cartopy.crs as ccrs
    import cartopy.feature as cfeature
    ...
    # TC initial position
    tc_init_lon_W = 360 - CONFIG['init_lon'] if CONFIG['init_lon'] > 180 else -CONFIG['init_lon']
    ax.plot(CONFIG['init_lon'], CONFIG['init_lat'],
           'g*', markersize=20, markeredgecolor='black', markeredgewidth=2,
           transform=ccrs.PlateCarree(),
           label=f'TC Initial Position\n({CONFIG["init_lat"]:.1f}°N, {abs(tc_init_lon_W):.1f}°W)',
           zorder=10)

    # Observation track
    if obs_track_truncated is not None:
        ax.plot(obs_track_truncated['lon'], obs_track_truncated['lat'],
    ...
    # Baseline forecast
    ax.plot(forecast_track_baseline['lon'], forecast_track_baseline['lat'],
           'o-', color='#9B59B6', markersize=6, linewidth=2,
           transform=ccrs.PlateCarree(),
           label='Baseline Forecast',
           zorder=6)

    # Trajectory-seeded forecast
    ax.plot(forecast_track_seeded['lon'], forecast_track_seeded['lat'],
           '*-', color='#E74C3C', markersize=10, linewidth=2,
           transform=ccrs.PlateCarree(),
           label='3-Site Trajectory Forecast',
           zorder=7)

    # Seeding trajectory sites
    for i, loc in enumerate(seeding_locations):
        ax.plot(loc['lon_center'], loc['lat_center'],
               '*', markersize=14, color='black',
               markeredgecolor='yellow', markeredgewidth=1.5,
               transform=ccrs.PlateCarree(),
               label='Trajectory Sites' if i == 0 else '',
               zorder=9)

        # Add site number label
        ax.text(loc['lon_center'], loc['lat_center'] - 0.5,
               f"S{i+1}", fontsize=9, fontweight='bold',
               ha='center', va='top', color='white',
               bbox=dict(boxstyle='round,pad=0.3', facecolor='black', alpha=0.7),
               transform=ccrs.PlateCarree(), zorder=10)

    ax.set_title('Ian 2022 - 3-Site Trajectory Seeding Test', fontsize=14, fontweight='bold', pad=10)
    ax.legend(loc='upper left', fontsize=11, framealpha=0.95)

    plt.tight_layout()
    track_plot = CONFIG['output_dir'] / "ian_ftle_track_comparison_targeted.png"
    plt.savefig(track_plot, dpi=150, bbox_inches='tight')
    plt.close()
    ...
    try:
        plot_seeding_locations_map(
            ...
            center_lat=None,  # No cluster center for trajectory sites
            center_lon=None
        )
    ...
    # =========================================================================
    print("\n" + "=" * 80)
    print("  ✓ 3-Site Trajectory Test Complete!")
    print("=" * 80)
    print()
    print("Summary:")
    print(f"  • Trajectory sites: {len(seeding_locations)}")
    print(f"  • Site configuration:")
    for i, site in enumerate(TRAJECTORY_SITES, 1):
        print(f"    - Site {i}: {site['lat']:.2f}°N, {site['lon']:.2f}°E ({site['radius_km']} km radius)")
    ...
    if deviations[-1] > 100:
        print("✓ STRONG SUCCESS: Trajectory seeding created significant track deviation!")
    elif deviations[-1] > 50:
        print("✓ MODERATE SUCCESS: Trajectory seeding improved track deviation")
    else:
        print("⚠️  LIMITED SUCCESS: Track deviation less than 50 km")
        print("   Consider adjusting site locations or trying 7-day lead time")
    print()
